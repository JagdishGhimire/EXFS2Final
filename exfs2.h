...
// A helper struct containing only the fixed-size fields
...
// Verify struct size at compile time using static_assert.
// This assertion should still pass with the revised calculation method.
static_assert(sizeof(Inode) == BLOCK_SIZE, "Inode size does not match BLOCK_SIZE");


// Constants derived from Inode structure 
...
// These Global Variables track the next segment index to potentially create.
...
void free_indirect_blocks(uint32_t block_num, int level); // Helper for freeing blocks recursively (levels 0 and 1)
