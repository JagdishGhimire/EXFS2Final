...
// Recursive helper to free indirect blocks (levels 0 and 1)
void free_indirect_blocks(uint32_t block_num, int level) {
    // Levels supported: 0 (single indirect), 1 (double indirect)
    ...
    for (int i = 0; i < POINTERS_PER_BLOCK; ++i) {
        if (pointers[i] != 0 && pointers[i] != UINT32_MAX) {
            if (level == 0) { // These are direct data blocks pointed to by the single indirect block
                free_data_block(pointers[i]);
            } else { // level == 1: These are single indirect blocks pointed to by the double indirect block
                free_indirect_blocks(pointers[i], level - 1); // Recurse to free single indirect blocks (level 0)
            }
        }
    }
...
int find_entry_in_dir(uint32_t dir_inode_num, const char *name, uint32_t *entry_inode_num) {
    ...
    // TODO: Iterate through direct, single, double blocks of the directory inode
    ...
    // If not found in direct blocks, continue search into single, double indirect blocks...
    // ... (Implementation needed using get_block_for_offset adapted for dirs) ...
...
int add_entry_to_dir(uint32_t dir_inode_num, const char *name, uint32_t entry_inode_num) {
    ...
    // TODO: Iterate through direct, single, double blocks
...
// Handles direct, single, double indirect blocks.
...
uint32_t get_block_for_offset(uint32_t inode_num, size_t offset, int allocate) {
    ...
    // --- 4. Triple Indirect ---
    // REMOVED Triple indirect block handling code
    // block_index -= double_indirect_limit; // No longer need to adjust
...
void list_recursive(uint32_t dir_inode_num, int depth, int debug_mode) {
    ...
    // TODO: Iterate through direct, single, double blocks of the directory
    ...
    if (error_occurred) {
        ...
        Inode cleanup_inode;
         if (read_inode(new_file_inode_num, &cleanup_inode) == 0) { // Read potentially partial inode
            ...
            // REMOVED: free_indirect_blocks(cleanup_inode.triple_indirect, 2);
         } else {
...
dir_scan_done:
          ...
          // REMOVED: free_indirect_blocks(current_inode.triple_indirect, 2);
